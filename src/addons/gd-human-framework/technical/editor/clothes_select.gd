...
func _ready() -> void:
	for key in CharEditGlobal.cloth_array.keys():
		var item = load("res://addons/gd-human-framework/technical/editor/clothes_item.tscn").instantiate()
...
func set_item(item: String, value: bool):
	if Engine.is_editor_hint() && get_tree().edited_scene_root is HumanCharacter:
		if value:
		...
		if value:
			get_parent().get_parent().get_parent().get_node("cam/spt/character").take_on_clothes(item)
		else:
			get_parent().get_parent().get_parent().get_node("cam/spt/character").take_off_clothes(item)
